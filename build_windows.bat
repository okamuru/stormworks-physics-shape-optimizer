...
set "RELEASE_DIR=release\Stormworks Physics Shape Optimizer 1.0.0 Alpha Windows"
...
"%BUILD_PYTHON%" -c "import shutil; shutil.make_archive(r'release\Stormworks Physics Shape Optimizer 1.0.0 Alpha Windows', 'zip', root_dir='release', base_dir=r'Stormworks Physics Shape Optimizer 1.0.0 Alpha Windows')"
if errorlevel 1 exit /b 1
"%BUILD_PYTHON%" -c "from pathlib import Path; import hashlib; p=Path(r'release\Stormworks Physics Shape Optimizer 1.0.0 Alpha Windows.zip'); print(hashlib.sha256(p.read_bytes()).hexdigest(), p)"
if errorlevel 1 exit /b 1
